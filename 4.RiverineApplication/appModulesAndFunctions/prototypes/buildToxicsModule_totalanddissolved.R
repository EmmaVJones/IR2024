               ...
               wellPanel(
                 ...
                 fluidRow(column(3,
                                 selectInput(ns('parameterChoice'), 'Choose a PWS parameter to investate further.',
                                             choices = c('Dissolved Nitrate', # remove IR2026
                                                         'Total Nitrate',
                                                         'Dissolved Chloride', # remove IR2026
                                                         'Total Chloride', 'Total Sulfate', 'Total Antimony', 'Total Arsenic', 'Total Barium',
...
toxicsSingleStation <- function(input,output,session, AUdata, stationData, WaterToxics, WCmetalsStationPWS, IntakeSites, markPCB, fishPCB, stationSelectedAbove){
  ...
  output$WCPWSRangeTableSingleSite <- DT::renderDataTable({req(stationData(), WaterToxics())
    if(is.na(unique(stationData()$PWS))){
      ...
      DT::datatable(PWSconcat, escape=F, rownames = F, options= list(scrollX = TRUE, pageLength = nrow(PWSconcat), dom='t'),
                    selection = 'none')  %>%
        formatStyle(c("PWS_NitrateDissolved_EXC", "PWS_NitrateDissolved_SAMP", "PWS_NitrateDissolved_STAT", "PWS_NitrateDissolved_MedianExceedance" ), "PWS_NitrateDissolved_STAT", backgroundColor = styleEqual(c('Review'), c('red'))) %>%
        ...
        formatStyle(c("PWS_ChlorideDissolved_EXC", "PWS_ChlorideDissolved_SAMP", "PWS_ChlorideDissolved_STAT", "PWS_ChlorideDissolved_MedianExceedance"),  "PWS_ChlorideDissolved_STAT", backgroundColor = styleEqual(c('Review'), c('red'))) %>%
  ...
  parameterResults <- reactive({req(!is.na(unique(stationData()$PWS)))
    tibble(Parameter = 'Dissolved Nitrate', Result = list(assessPWS(stationData(), NITROGEN_NITRATE_DISSOLVED_00618_mg_L, LEVEL_00618, 10))) %>% 
      bind_rows(tibble(Parameter = 'Total Nitrate', Result = list(assessPWS(stationData(), NITROGEN_NITRATE_TOTAL_00620_mg_L, LEVEL_00620, 10)))) %>% 
      bind_rows(tibble(Parameter = 'Dissolved Chloride',  Result = list(assessPWS(stationData(), CHLORIDE_DISSOLVED_00941_mg_L, LEVEL_00941, 250)))) %>% 
