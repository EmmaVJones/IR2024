  ...
  tagList(
    wellPanel(
      ...
      fluidRow(column(2,uiOutput(ns('oneStationSelectionUI'))),
               column(1),
               column(2,br(),checkboxInput(ns('displayBSAcolors'), 'Display Benthic Stressor Analysis Colors on Plot (Applicable to dissolved nitrate)', value = FALSE)),
               column(1),
...
NitratePlotlySingleStation <- function(input,output,session, AUdata, stationSelectedAbove){
  ...
  output$plotly <- renderPlotly({  req(input$oneStationSelection, oneStation())
    ...
    if(input$displayBSAcolors == TRUE){
      ...
      # for IR 2024 we will use dissolved and total, but IR 2026+ only total for PWS
