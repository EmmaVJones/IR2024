...
assessmentWindowLowFlowsBasinLevelFlag <- dplyr::select(assessmentWindowLowFlows, Date, `7Q10 Flag`, BASIN_CODE, BASIN_NAME) %>% 
  group_by(Date, BASIN_CODE) %>% 
  #mutate(n = n()) %>% # visualize how many duplicated here
  #arrange(desc(n), Date) %>% 
  mutate(n = 1:n()) %>% 
  filter(n ==1 ) %>% 
  dplyr::select(-n)
