...
# Note: these get a cut off when bios are "done" assessing for all cycles starting in IR2024. When the stations table is run the first X many times, we want to source all SCI data, but after the deadline (not the same every cycle), we need to change this to the local dataset archived for the specific IR
...
# if you want to know the ith number of a particular station, use this: i = which(stationTable$STATION_ID == '4AROA175.63', arr.ind=TRUE)
...
write_csv(stationTableResults, 'processedStationData/stationTableResults05192023.csv',
