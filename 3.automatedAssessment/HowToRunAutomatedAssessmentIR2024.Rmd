...
historicalStationsTable2 <- read_excel('data/oldData/tbl_ir_mon_stations2018IRfinal.xlsx')
```
...
We only want to run riverine methods on riverine stations right now, so identify all stations that have lake AU designations. We will use this information later in the assessment analysis.
...
#i = 1
  ...
  stationData <- filter(conventionals, FDT_STA_ID %in% stationTable$STATION_ID[i]) %>%
    #stationData <- filter(conventionals, FDT_STA_ID == "4ATKR000.08") %>%# test a specific station
