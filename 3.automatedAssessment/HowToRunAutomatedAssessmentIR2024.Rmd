...
lakeStationsImportant <- dplyr::select(lakeStations, STATION_ID:ID305B_3, NUT_TP_EXC:NUT_CHLA_STAT, WQS_ID, GNIS_Name,
...
# lakePolyWQS <- st_join(lakeStationsMoreWork, lakesPoly, join = st_intersects) %>%
#     filter(!is.na(OBJECTID)) %>% 
#   dplyr::select(StationID = STATION_ID, WQS_ID = WQS_ID.y, TYPE_1) %>% 
#   mutate(`Buffer Distance` = 'In Polygon', 
#          Comments = 'EVJ Addition') %>% 
#   st_drop_geometry()
#  
# WQSlookup1 <- pin_get('ejones/WQSlookup', board = 'rsconnect')
# citmonWQS1 <- pin_get("ejones/citmonStationsWithWQSFinal", board = "rsconnect")
# 
# WQSlookup2 <- bind_rows(WQSlookup1, 
#                         filter(lakePolyWQS, TYPE_1 %in% c('C', 'L')) %>% #View()
#                           dplyr::select(-TYPE_1))
# 
# citmonWQ2 <- bind_rows(citmonWQS1, 
#                         filter(lakePolyWQS, ! TYPE_1 %in% c('C', 'L')) %>% #View()
#                           dplyr::select(-TYPE_1))
# 
# pin(WQSlookup2, 'ejones/WQSlookup', board = 'rsconnect', 
#     description = 'WQS lookup table from metadata attribution application')
# 
# 
# pin(citmonWQ2, 'ejones/citmonStationsWithWQSFinal', board = 'rsconnect', 
...
#     description = "Final Conventionals data for IR2024 with added 7Q10 flag joined at major river basin level. This is used for the shiny app versions of the automated assessment protocol to save rendering time", board = 'rsconnect')
```
