...
## Water Intake Analyses

New for IR2024, all VDH water intakes are pulled from DEQ's GIS REST service, buffered by 100m, and spatially joined to stations within the assessment cycle to identify station proximity for PWS intake assessments. All scripts can be found in waterIntakeAnalysis.Rmd in this directory.


## Low Flow Analyses

New for IR2024, all low flow statistics are generated using the same methods (in R) as the Water Permitting program to maintain consistency within the agency. The 7Q10/Assessment Low Flow Analysis.Rmd walks users through pulling and analyzing this data from NWIS using functions built by Connor Brogan (and modified for faster assessment output by Emma Jones).

This analysis must be run after April 1 of any given year to incorporate the whole prior water year. For IR 2024, these analyses were run on April 3, 2023 by Emma Jones and made available to the assessment products.
...
Final citmon non agency data from 2017-2022.
...
stationsToDo <- filter(conventionals_distinct, ! FDT_STA_ID %in% stationsTable2024begin$STATION_ID) %>% 
  ...
  mutate(# throw in a flag for swamp but still need to force it to a waterbody type the assessment tools understand
         WATER_TYPE = case_when(#WATER_TYPE == 'SWAMP' ~ paste(TYPE_1, 'SWAMP', sep = ': '),
                             WATER_TYPE %in% c('STREAM', 'SWAMP', 'SPRING', 'CANAL') ~ 'RIVER',
                             WATER_TYPE %in% c('ESTURY', 'OCEAN') ~ 'ESTUARY',
                             WATER_TYPE %in% c('RESERV', 'LAKE') ~ 'RESERVOIR',
                             WATER_TYPE %in% c(NA,'', 'UNK') ~ NA_character_,
...
stationsLeft <- st_join(stationsToDo, vahu6) %>% 
  ...
  left_join(dplyr::select(AUlookup, FDT_STA_ID, ID305B_1, ID305B_2, ID305B_3, ID305B_4, ID305B_5, ID305B_6, ID305B_7, ID305B_8, ID305B_9,  ID305B_10), 
            by = c('STATION_ID' = 'FDT_STA_ID')) %>% # join in AU info from the pinned AUlookup dataset (populated by assessors in metadata attribution app)
  distinct(STATION_ID, .keep_all = T)
...
For a few new stations, assessors provided Lacustrine and TYPE designation. Grab that now and add in.

```{r}
allExtra <- read_excel('C:/Users/wmu43954/OneDrive - Commonwealth of Virginia/WQA/ForReview.xlsx',
                       sheet = 'BRRO') %>% dplyr::select(FDT_STA_ID, TYPE_1, Lacustrine) %>% 
  bind_rows(read_excel('C:/Users/wmu43954/OneDrive - Commonwealth of Virginia/WQA/ForReview.xlsx',
                       sheet = 'CO') %>% dplyr::select(FDT_STA_ID, TYPE_1, Lacustrine)) %>% 
  bind_rows(read_excel('C:/Users/wmu43954/OneDrive - Commonwealth of Virginia/WQA/ForReview.xlsx',
                       sheet = 'NRO') %>% dplyr::select(FDT_STA_ID, `NRO CORRECTED STATION ID`, TYPE_1, Lacustrine) %>% 
              mutate(FDT_STA_ID = ifelse(!is.na(`NRO CORRECTED STATION ID`), `NRO CORRECTED STATION ID`, FDT_STA_ID)) %>% 
              dplyr::select(-`NRO CORRECTED STATION ID`)) %>% 
  # bonus local data
  bind_rows(read.csv('data/Book1.csv') %>% dplyr::select(FDT_STA_ID, TYPE_1)) %>% 
  
  bind_rows(read_excel('C:/Users/wmu43954/OneDrive - Commonwealth of Virginia/WQA/ForReview.xlsx',
                       sheet = 'PRO') %>% dplyr::select(FDT_STA_ID, TYPE_1, Lacustrine)) %>% 
  bind_rows(read_excel('C:/Users/wmu43954/OneDrive - Commonwealth of Virginia/WQA/ForReview.xlsx',
                       sheet = 'TRO') %>% dplyr::select(FDT_STA_ID, TYPE_1, Lacustrine)) %>% 
  bind_rows(read_excel('C:/Users/wmu43954/OneDrive - Commonwealth of Virginia/WQA/ForReview.xlsx',
                       sheet = 'VRO') %>% dplyr::select(FDT_STA_ID, TYPE_1, Lacustrine)) 


stationsTable2024begin <- left_join(stationsTable2024begin, allExtra, by = c("STATION_ID" = 'FDT_STA_ID')) %>% 
  mutate(LACUSTRINE = coalesce(Lacustrine, LACUSTRINE),
         TYPE_1 = coalesce(TYPE_1.y, TYPE_1.x)) %>% 
  dplyr::select(names(stationsTable2024begin))
```
