...
Bring in and organize BRRO citmonNonagency sites.

```{r BRRO citmonNonagency}
...
county <- st_read('C:/HardDriveBackup/R/GitHub/pinData/data/GIS/VACountyBoundaries.shp')
...
citmonNonAgency_sf1 <- citmonNonAgency_sf %>% 
  ...
  st_intersection(dplyr::select(county, CountyCityName = NAME, FIPS)) %>% 
...
distinctSites_sf <- bind_rows(distinctSites_sf %>% st_drop_geometry(),
                              citmonNonAgency_sf1 %>% st_drop_geometry() %>% 
...
WQStable <- snapAndOrganizeWQS(distinctSitesToDoWQS, 'FDT_STA_ID', riverine, 
                               ...
                               WQStable)
#saveRDS(WQStable, 'data/WQStableBRROCitmon.RDS')
