...
This project is a outlines how to preprocess data for the manual WQS and AU attribution process as well as organize citizen monitoring and conventionals data for IR 2024.
...
It is important to troubleshoot and develop as much as possible prior to the final data pull. Below is how the process looks with a final data pull; however, most of these methods were developed on draft datasets in order to complete as much of the assessment attribution prior to final data pulls. The draft versions of this document and workflow are available in draftMethodIterations/ .
...
#saveRDS(distinctSites_sf, './data/distinctSites_sf04132023.RDS')
```
...
Reid, Joe, and Emma organized citizen monitoring data for 2017-2022. The citmon-data-management repo (a copy saved in GitLab/WQA) is how we smashed together the results.

```{r citmon data}
citmon <- pin_get("ejones/citmonNonAgencyDataIR2024", board = "rsconnect") %>% 
 dplyr::select(FDT_STA_ID, GROUP_STA_ID, STA_DESC, Data_Source, Latitude, Longitude) %>% 
  distinct(FDT_STA_ID, .keep_all = T)
```

Add these to the distinctSites_sf dataset

```{r distinctSites_sf update with citmon}
citmonNonAgency_sf <- filter(distinctSites_sf, is.na(ASSESS_REG)| is.na(US_L3NAME)) %>%  st_drop_geometry() %>% 
  bind_rows(citmon) %>% 
...
distinctSites_sf <- readRDS('./data/distinctSites_sf_postassessmentLayerSubbasinJoin04132023.RDS')#distinctSites_sf_withCitmon03072023.RDS') #distinctSites_sf_withCitmon02232023.RDS')
...
### Pin Conventionals Distinct to server
...
```{r}
# pin(distinctSites_sf, 'ejones/conventionals2024_distinct', board = 'rsconnect',
#     description = 'Final IR2024 conventionals_distinct dataset based on conventionals final pull from Roger and 2017-2022 citmon/nonagency data')  # see below for fix coming this way at final stage

rm(citmon, citmonNonAgency_sf, conventionals, stationHitList, distinctSites)
```
