...
First, bring in Cleo's final stations data from 2022 cycle. 
...
# double check the math works out
nrow(distinctSites_AU) + nrow(distinctSites_AUtoDo) == nrow(distinctSites_AUall)
```


Next we need to side step to join 1a stations (from final IR2022 spatial layer) that really are in CEDS as 1A.
