...
```{r conventionals, echo=F, warning=F, error=FALSE}
...
conventionals <- conventionals %>% 
  dplyr::select(FDT_STA_ID, STA_DESC, Data_Source, Latitude, Longitude) %>% 
  distinct(FDT_STA_ID, .keep_all = T)

distinctSites <- dplyr::select(conventionals, -c(Latitude, Longitude)) # object to save unique sites for later, use standardized other lat/lng
```
...
#saveRDS(distinctSites_sf, './data/distinctSites_sf02232023.RDS')
```
...
Bring in and organize citmonNonagency sites. Below is for 2019-2020 data from Reid.

```{r reid citmon data}
citmonNonAgency <- read.csv('data/draftData/CMC_FOSR_19_20.csv')

# force into conventionals format
#source('preprocessingModules/citizenMonitoringDataToConventionals.R')
...
citmonNonAgency_sf <- citmonNonAgency %>% 
  dplyr::select(FDT_STA_ID, STA_DESC, Data_Source,
                Latitude, Longitude) %>% 
  distinct(FDT_STA_ID, .keep_all = T) %>% 
   st_as_sf(coords = c("Longitude", "Latitude"),  # make spatial layer using these columns
           remove = F, # don't remove these lat/lon cols from df
...
#     description = 'Working IR2024 conventionals_distinct dataset based on 02162023 conventionals draft pull from Roger and 2019-2020 citmon/nonagency data from Reid that have FDT_STA_ID')  # see below for fix coming this way at final stage
...
```{r}
#source('specialStepForIR2024toImportWQSinfoAttributedAtAUlevelFromODS.R')
```
