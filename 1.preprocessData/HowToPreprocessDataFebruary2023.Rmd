...
```{r conventionals}
conventionalsRaw <- read_excel('data/draftData/CEDSWQM_2024_IR_20230209.XLSX', sheet = 'CONVENTIONALS') 

conventionals <-  mutate(conventionalsRaw, #Source_Sta_Id = as.character(NA),
                         Data_Source = 'DEQ') %>%
  dplyr::select(FDT_STA_ID, STA_DESC, Data_Source, FDT_DATE_TIME, Latitude, Longitude)
rm(conventionalsRaw)
```
...
```{r}
# pin final versions
# pin('ejones/conventionals2024draft', board = 'rsconnect')
...
Throughout the metadata process, there will be times when you need to grab the attributed data from teh server, organize it, and push an app update. You first need to go to the server and grab all data attributed so far from the AUlookupTable and WQSlookupTable directories in the currently active application instance (see below for more info).
