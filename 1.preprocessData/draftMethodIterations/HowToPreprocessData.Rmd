...
citmonNonAgency_sf1 <- citmonNonAgency_sf %>% 
...
distinctSites_sf <- bind_rows(distinctSites_sf %>% st_drop_geometry(),
  ...
  dplyr::select(names(distinctSites_sf)) %>% # once you have the right column names, reorder to correct order
  st_as_sf(coords = c("Longitude", "Latitude"),  # make spatial layer using these columns
           remove = F, # don't remove these lat/lon cols from df
           crs = 4326) %>% 
  distinct(FDT_STA_ID, .keep_all = T)
...
saveRDS(distinctSites_sf, 'data/distinctSites_sf_withCitmon.RDS')
