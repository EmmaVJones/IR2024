...
schemaFin <- read_csv('data/citmonnonagencydummydata_EVJ.csv') %>%# email 1/8/21, EVJ added random LEVEL info for testing
  rename('CHLOROPHYLL_A_ug_L' = 'CHLOROPHYLL_A_mg_L', 
         ...
         # new for IR2024
         VADEQ_ADMIN_REGION = Deq_Region, 
         MONITORING_REGION = STA_REC_CODE,
         HARDNESS_TOTAL_00900_mg_L = `HARDNESS_TOTAL_00900_mg_L`,# `HARDNESS_TOTAL_00900_mg/L`,
         RMK_HARDNESS_TOTAL = RMK_00900,# no remark field included in IR2024 version of conventionals
...
conventionals <- conventionalsRaw %>% 
  ...
  mutate( # new for IR2024, this is provided in conventionals pull
         ...
         RMK_TDNLF = as.character(NA), #not provided in IR2024 
         ...
         RMK_HARDNESS_TOTAL = as.character(NA),# no remark field included in IR2024 version of conventionals
         LEVEL_HARDNESS_TOTAL = as.factor(NA), 
         ...
         ECOLI = `E.COLI_ECOLI_CFU/100mL`, 
         # IR2022 this was calculated, but IR2024 this is apparently not calculated
         ##`ECOLI_CFU/100mL`,  # final conventionals have this calculated now
         RMK_ECOLI = RMK_ECOLI, #ECOLI_RMK,
...
# for bookdown
#saveRDS(schemaFin[1:5,], 'schemaFin.RDS')
